...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "scattered/trapped data + workflow friction + missing capability (teaching) + decision paralysis"
    - topic: "primary persona scope"
      decision: "cloud developers broadly (AWS/Azure/GCP) — multi-user product"
    - topic: "core insight / differentiator"
      decision: "questions are AI-generated on demand per exam + chosen count; not a fixed static bank"
    - topic: "auth model"
      decision: "email + password and OAuth login; accounts persist per-user session history"
    - topic: "role model"
      decision: "flat — every user is a learner; no admin/author roles in MVP"
    - topic: "MVP scope vs timeline"
      decision: "full flow kept (incl. progress dashboard) at an aggressive 2-week target; user accepted; AI generation quality flagged as the main risk"
  frs_drafted: 11
  quality_check_status: accepted
---

<!-- Shape notes — greenfield. Body anticipates the 10 PRD sections in order.
     Captured by /10x-shape; consumed by /10x-prd. -->
...
<!-- Timeline note: full flow retained (incl. progress dashboard) at a 2-week
     after-hours target. User accepted on 2026-06-17. AI question/answer/explanation
     generation quality flagged as the primary delivery risk. -->
...
<!-- Note: answer/explanation accuracy and per-user privacy are committed as
     Guardrails under Success Criteria rather than duplicated here. Cross-device
     browser support was considered and not committed as a v1 NFR. -->
...
1. **What is the maximum question count per session?** — Owner: user. A cap is needed to keep generation responsive and affordable (relates to FR-004 / FR-005).
...
Ran 2026-06-17. Status: **accepted** — no gaps. All required greenfield elements
present (Access Control, one-sentence Business Logic, project artifacts, timeline-cost
within the 2-week target, Non-Goals). Preserved-behavior check is n/a for greenfield.
Two deferrals were routed to Open Questions (max question count; cross-exam progress
metric) rather than left as gaps.
