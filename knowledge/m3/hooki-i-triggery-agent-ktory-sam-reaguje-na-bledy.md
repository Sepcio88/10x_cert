...
- **Codex**: `PostToolUse` w `.codex/hooks.json` ([dokumentacja](https://openai.com/codex/docs/hooks))
- **Copilot**: hooki w `.github/hooks/*.json`; w VS Code Copilot jest kompatybilny z formatem `.claude/settings.json` ([dokumentacja](https://docs.github.com/en/copilot/customizing-copilot/extending-copilot-coding-agent/configuring-coding-agent-hooks))
...
- Copilot Cloud Agent (wersja chmurowa) ma najwęższy zestaw możliwości: krótkotrwały sandbox, `ask` traktowany jako `deny`.
...
- [Codex Hooks](https://openai.com/codex/docs/hooks) — dokumentacja hooków: zdarzenia, hash-based trust model, ograniczenia
...
- Prework [\[2.4\]](https://platforma.przeprogramowani.pl/external/10xdevs-3-prework/pl/07) _Agent-Native IDE_ — dyscyplina bezpieczeństwa (czysty repo, testy, review, diffy), którą hooki automatyzują
- Prework [\[1.3\]](https://platforma.przeprogramowani.pl/external/10xdevs-3-prework/pl/03) _Jak uczyć się i rozwijać z AI_ — tryb korepetytora, tutaj zastosowany do hooka, który uczy agenta reagować na błędy
- Prework [\[2.2\]](https://platforma.przeprogramowani.pl/external/10xdevs-3-prework/pl/05) _Cursor — Podstawy operacyjne_ i [\[2.3\]](https://platforma.przeprogramowani.pl/external/10xdevs-3-prework/pl/06) _Claude Code — Podstawy operacyjne_ — podstawy konfiguracji narzędzi, na których opiera się konfiguracja hooków
